...
NTSTATUS PerformKernelNativeInject(_In_ PKERNEL_NATIVE_INJECT_REQUEST Request) {
    ...
    log("Successfully attached to process space of PID %lu for Native Injection.", Request->TargetPid);
...
NTSTATUS PerformKernelManualMap(_In_ PKERNEL_MANUAL_MAP_REQUEST Request) {
    ...
    log("Successfully attached to process space of PID %lu for Manual Map.", Request->TargetPid);
...
NTSTATUS DriverEntry(_In_ PDRIVER_OBJECT DriverObject, _In_ PUNICODE_STRING RegistryPath) {
    ...
    RtlInitUnicodeString(&driverName, L"\\Driver\\CpuPlusPlus");
